...
# Cars have data, like number of wheels, number of doors, and seating capacity They also 
# exhibit behavior: they can accelerate, stop, show how much fuel is left, 
...
# Like cars. All the same make and model (and all have an engine, wheels, doors, and so on). 
...
# (as we talked about in the theory section). As an example, a vehicle class has its
#  own attributes that define what objects are vehicles. The number of wheels, type of tank, 
# seating capacity, and maximum velocity are all attributes of a vehicle.

# We define classes with a class statement
class Vechile:
    pass 
...
# Here car is an object (or instance) of the class Vehicle.
 ...
 # we define our class to receive data when it initiates it:
 class Vehicle:
...
# So when we create the vehicle object, we can define these attributes. 
# Imagine that we love the Tesla Model S, and we want to create this kind of object. 
# It has four wheels, runs on electric energy, has space for five seats, 
# and the maximum velocity is 250km/hour (155 mph). Let’s create this object:

tesla_model_s = Vehicle(4, 'electric', 5, 250)

# This is an implementation of two methods: number_of_wheels and set_number_of_wheels. 
# We call it getter & setter. Because the first gets the attribute value, 
